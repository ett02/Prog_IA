...
def solve_schedule():
    ...
    for w in all_workers:
        for d in range(n_days):
            for s in shifts:
                model.Add(shift_vars[(w, d, s)] * preference_weights[w][s] + 
                          shift_vars[(w, d, s)] * (2 if d == preferred_rest_day.get(w, -1) else 0) +
                          max(0, 5 - holiday_tolerances[w]) * sum(shift_vars[(w, d, s)] for s in shifts if days[d] in ["2026-12-08", "2026-12-25", "2026-12-26", "2027-01-01", "2027-01-06"]) <= objective)
