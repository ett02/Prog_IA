...
def solve_schedule():
    ...
    for w in all_workers:
        w_prefs = preference_weights.get(w, {})
        for d in range(n_days):
            if d == 25:  # 2026-12-25 (Natale)
                pen = w_prefs.get("night", 3)  # penalità per lavoratori con tolleranza notturna bassa
                if pen > 0:
                    penalty_terms.append(shift_vars[(w, d, "night")] * pen)
            elif d == 26:  # 2026-12-26 (Natale)
                pen = w_prefs.get("morning", 3)  # penalità per lavoratori con tolleranza mattutina bassa
                if pen > 0:
                    penalty_terms.append(shift_vars[(w, d, "morning")] * pen)
            elif d == 31:  # 2026-12-31 (Capodanno)
                pen = w_prefs.get("afternoon", 3)  # penalità per lavoratori con tolleranza pomeridiana bassa
                if pen > 0:
                    penalty_terms.append(shift_vars[(w, d, "afternoon")] * pen)

    # Penalità per violazioni del giorno di riposo preferito
    for w in all_workers:
        w_prefs = preference_weights.get(w, {})
        preferred_rest_day = w_prefs.get("preferred_rest_day", None)
        if preferred_rest_day is not None and preferred_rest_day != 6:  # non è sabato
            for d in range(n_days):
                if d == preferred_rest_day:
                    pen = 1  # penalità per violazione del giorno di riposo preferito
                    penalty_terms.append(shift_vars[(w, d, "morning")] * pen)
                else:
                    pen = 0  # penalità 0 per giorni non di riposo
                    penalty_terms.append(shift_vars[(w, d, "morning")] * pen)

    # Penalità per turni festivi per worker con bassa tolleranza (continuazione)
    for w in all_workers:
        w_prefs = preference_weights.get(w, {})
        for s in shifts:
            if s == "night":
                pen = w_prefs.get("night_tol", 3)  # penalità per lavoratori con tolleranza notturna bassa
                if pen > 0:
                    penalty_terms.append(shift_vars[(w, d, s)] * pen)
            elif s == "morning":
                pen = w_prefs.get("holiday_tol", 4)  # penalità per lavoratori con tolleranza mattutina alta
                if pen > 0:
                    penalty_terms.append(shift_vars[(w, d, s)] * pen)

    # Penalità per turni non preferiti (continuazione)
    for w in all_workers:
        w_prefs = preference_weights.get(w, {})
        for d in range(n_days):
            for s in shifts:
                if s == "night":
                    pen = w_prefs.get("holiday_tol", 4)  # penalità per lavoratori con tolleranza notturna alta
                    if pen > 0:
                        penalty_terms.append(shift_vars[(w, d, s)] * pen)
                elif s == "morning":
                    pen = w_prefs.get("night_tol", 3)  # penalità per lavoratori con tolleranza mattutina bassa
                    if pen > 0:
                        penalty_terms.append(shift_vars[(w, d, s)] * pen)
