...
def solve_schedule():
    ...
    for w in all_workers:
        w_prefs = preference_weights.get(w, {})
        for d in range(n_days):
            if d == 25:  # venerdì (festa)
                if 'holiday_tolerance' not in w_prefs or wPrefs['holiday_tolerance'] < 2:
                    penalty_terms.append(shift_vars[(w, d, "night")] * 5)  # penalità alta
            elif d == 26:  # sabato (festa)
                if 'holiday_tolerance' not in w_prefs or wPrefs['holiday_tolerance'] < 2:
                    penalty_terms.append(shift_vars[(w, d, "morning")] * 5)  # penalità alta
            elif d == 31:  # domenica (festa)
                if 'holiday_tolerance' not in w_prefs or wPrefs['holiday_tolerance'] < 2:
                    penalty_terms.append(shift_vars[(w, d, "afternoon")] * 5)  # penalità alta

    # Penalità per violazioni del giorno di riposo preferito
    for w in all_workers:
        w_prefs = preference_weights.get(w, {})
        if 'preferred_rest_day' not in w_prefs or wPrefs['preferred_rest_day'] is None:
            continue
        for d in range(n_days):
            if d == wPrefs['preferred_rest_day']:
                penalty_terms.append(shift_vars[(w, d, "morning")] * 3)  # penalità media

    # Penalità per turni non preferiti (altri criteri soft)
    for w in all_workers:
        w_prefs = preference_weights.get(w, {})
        if 'preferred_shifts' not in w_prefs or len(wPrefs['preferred_shifts']) == 0:
            continue
        for d in range(n_days):
            for s in shifts:
                if (w, d, s) not in wPrefs['preferred_shifts']:
                    penalty_terms.append(shift_vars[(w, d, s)] * 2)  # penalità bassa
