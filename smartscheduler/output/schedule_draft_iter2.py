...
def solve_schedule():
    ...
    for w in all_workers:
        w_prefs = preference_weights.get(w, {})
        for d in range(n_days):
            if date.fromisoformat(days[d]) == date(2026, 12, 25) or date.fromisoformat(days[d]) == date(2027, 1, 1): # festivi
                pen = w_prefs.get("night", 3)
                if pen > 0:
    ...
    for w in all_workers:
        ...
        if preferred_rest_day is not None and sum(shift_vars[(w, d, s)] for d in range(n_days) for s in shifts) != 0: # non è un giorno di riposo
            penalty_terms.append(shift_vars[(w, (date.fromisoformat(days[d]) - date(2026, 12, 25)).weekday(), "morning")] * abs(preferred_rest_day - ((date.fromisoformat(days[d]) - date(2026, 12, 25)).weekday()))
    ...
    for w in all_workers:
        ...
        for d in range(n_days):
            for s in shifts:
                if w_prefs.get(s, 1) == 0: # non è un turno preferito
                    penalty_terms.append(shift_vars[(w, d, s)])
