...
def solve_schedule():
    ...
    for w, preferred_rest_day in preferred_rest_day.items():
        for d in range(n_days):
            if date.fromisoformat(days[d]) % 7 == preferred_rest_day:
                penalty_terms.append(shift_vars[(w, d, "morning")] * 1)  # penalità per mattino

    # Penalità aggiuntive (seleziona una o più)
    # - Penalità per turni non preferiti
    penalty_terms.append(sum(preference_weights["W06"][s] for s in shifts))
