...
def solve_schedule():
    ...
    for w in all_workers:
        w_prefs = preference_weights.get(w, {})
        for d in range(n_days):
            if date.fromisoformat(days[d]) % 7 == 0:  # sabato o domenica
                pen = w_prefs.get("night", 3)  # default: penalità 3 (da evitare)
                if pen > 0:
                    penalty_terms.append(shift_vars[(w, d, "night")] * pen)

    # Penalità per violazioni del giorno di riposo preferito
    for w, preferred_rest_day in preferred_rest_day.items():
        for d in range(n_days):
            if date.fromisoformat(days[d]) % 7 == preferred_rest_day:  # giorno di riposo preferito
                pen = preference_weights.get(w, {}).get("night", 3)  # default: penalità 3 (da evitare)
                if pen > 0:
                    penalty_terms.append(shift_vars[(w, d, "night")] * pen)

    # Penalità per turni non preferiti con tolleranza bassa
    for w in all_workers:
        w_prefs = preference_weights.get(w, {})
        for s in shifts:
            if w_prefs.get(s, 1) > 2:  # tolleranza bassa
                pen = w_prefs.get(s, 1)  # default: penalità 1 (neutro)
                if pen > 0:
                    penalty_terms.append(shift_vars[(w, d, s)] * pen)
