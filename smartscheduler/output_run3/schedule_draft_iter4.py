...
def solve_schedule():
    ...
    for w in all_workers:
        ...
        if preferred_rest_day_w is not None:
            for d in range(n_days):
                if d == preferred_rest_day_w:  # giorno di riposo preferito
                    pen = 2  # penalità maggiore per violazioni del giorno di riposo
                else:
                    pen = 1  # penalità standard per giorni non di riposo
                penalty_terms.append(shift_vars[(w, d, "morning")] * pen)

    # Penalità aggiuntive per altri criteri soft rilevanti (ad esempio, turni consecutivi)
    # TODO: Aggiungi qui penalità aggiuntive per:
    # - Turni festivi con bassa tolleranza
