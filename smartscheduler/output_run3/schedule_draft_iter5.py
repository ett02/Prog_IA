...
def solve_schedule():
    ...
    for w in all_workers:
        tol = night_tolerances.get(w, 3)  # default 3 se non specificato
        if tol < 4:  # tolleranza bassa
            for d in range(n_days):
                for s in shifts:
                    if s == "night" and shift_hours[s] > 12:
                        pen = preference_weights.get(w, {}).get(s, 1) * (tol + 1)
                        penalty_terms.append(shift_vars[(w, d, s)] * pen)

    # Penalità per violazioni del giorno di riposo preferito
    for w in all_workers:
        preferred_rest_day_w = preferred_rest_day.get(w, None)
        if preferred_rest_day_w is not None:
            for d in range(n_days):
                if date.fromisoformat(days[d]) % 7 == preferred_rest_day_w:
                    pen = preference_weights.get(w, {}).get("afternoon", 1) * (1 + 1)
    ...
    for w in all_workers:
        ...
        for d in range(n_days):
            for s in shifts:
                if w_prefs.get(s, 1) > 0:  # se non è neutro
                    pen = w_prefs[s]
                    penalty_terms.append(shift_vars[(w, d, s)] * pen)
