...
def solve_schedule():
    ...
    # Penalità per turni festivi
    for w in all_workers:
        w_prefs = preference_weights.get(w, {})
        for d in range(n_days):
            day = date.fromisoformat(days[d])
            if day.weekday() == 5:  # sabato
                pen = w_prefs.get("night", 1)  # default: penalità 1 (neutro)
                if pen > 0:
    ...
    for w in all_workers:
        ...
        if preferred_rest_day_w is not None:
            day = date.fromisoformat(days[preferred_rest_day_w])
            for d in range(n_days):
                if day.weekday() == preferred_rest_day_w and shift_vars[(w, d, "morning")] and shift_vars[(w, d, "afternoon")]:
                    penalty_terms.append(shift_vars[(w, d, "morning")] + shift_vars[(w, d, "afternoon")] * 2)

    # Penalità per turni non preferiti in caso di violazione del giorno di riposo
    for w in all_workers:
        w_prefs = preference_weights.get(w, {})
        preferred_rest_day_w = preferred_rest_day.get(w, None)
        if preferred_rest_day_w is not None:
            ...
            for d in range(n_days):
                if day.weekday() == preferred_rest_day_w and shift_vars[(w, d, "morning")] and shift_vars[(w, d, "afternoon")]:
                    penalty_terms.append(shift_vars[(w, d, "night")] * 2)

    # Modello con penalità aggiuntive
