...
def solve_schedule():
    ...
    night_tolerances["W12"] = 2
    unavailable_dates["W12"] = {18}
    ...
    #    IMPORTANTE: min_nights <= max_nights sempre (evita contraddizioni)
    GLOBAL_MIN_NIGHTS = 3
    for w in all_workers:
        tol = night_tolerances.get(w, 3)  # default 3 se non specificato
        max_nights_for_w = min(7, 2 + tol)
        # min dinamico: mai più del massimo (evita INFEASIBLE per bassa tolleranza)
        min_nights_for_w = min(GLOBAL_MIN_NIGHTS, max_nights_for_w)
        model.Add(
            sum(shift_vars[(w, d, "night")] for d in range(n_days)) >= min_nights_for_w
        )
        model.Add(
    ...
    # Penalità per turni festivi (TODO LLM)
    festive_dates = [date(2026, 12, 8), date(2026, 12, 25), date(2026, 12, 26), date(2027, 1, 1), date(2027, 1, 6)]
    for w in all_workers:
        tol = night_tolerances.get(w, 3)  # default 3 se non specificato
        if tol <= 2:  # bassa tolleranza
            for d in range(n_days):
                if date.fromisoformat(days[d]) in festive_dates:
                    for s in shifts:
